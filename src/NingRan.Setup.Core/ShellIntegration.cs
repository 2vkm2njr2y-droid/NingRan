...
public static class ShellIntegration
{
    ...
    [
        new(".nrenc", "NingRan.EncryptedFile", "凝然加密文件"),
        new(".nrid", "NingRan.IdentityBackup", "凝然身份备份"),
        new(".nrpub", "NingRan.PublicIdentity", "凝然公开身份"),
    ];
        ...
        if (state.FileAssociations)
        {
            foreach (var association in Associations)
            {
                ...
                using (var iconKey = Registry.CurrentUser.CreateSubKey(
                           ...
                           writable: true))
                {
                    iconKey.SetValue(null, $"\"{executable}\",0", RegistryValueKind.String);
                }
